...
async function timedFetch(url, init = {}, timeout = Number(env.REQUEST_TIMEOUT_MS || 12000)) {
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), timeout);
    const started = Date.now();
    try {
        const r = await fetch(url, { ...init, signal: controller.signal, headers: { 'user-agent': 'RakshaMap/2.0 SIH disaster decision support', ...(init.headers || {}) } });
        const latency = Date.now() - started;
        if (!r.ok)
            throw new Error(`${r.status} ${r.statusText}`);
        return { response: r, latency };
    }
    finally {
        clearTimeout(timer);
    }
}
function setSource(s) { sourceStatus.set(s.id, s); }
function baseSource(id, name, authority, official, url) { return { id, name, authority, official, url, status: 'optional', updatedAt: null, detail: '' }; }
const sources = [
...
async function fetchEarthquakes() {
    ...
    const d = await response.json();
    setSource({ ...sourceById.usgs, status: 'live', updatedAt: new Date().toISOString(), latencyMs: latency, detail: 'USGS all-day earthquake feed connected.' });
    return (d.features ?? []).map((f) => ({ id: String(f.id), lat: Number(f.geometry?.coordinates?.[1]), lon: Number(f.geometry?.coordinates?.[0]), magnitude: Number(f.properties?.mag ?? 0), place: String(f.properties?.place ?? 'Unknown'), time: new Date(Number(f.properties?.time ?? 0)).toISOString() })).filter((q) => q.lat >= 5 && q.lat <= 38 && q.lon >= 67 && q.lon <= 100);
}
async function fetchEonet() {
    const { response, latency } = await timedFetch(EONET);
    const d = await response.json();
    setSource({ ...sourceById.eonet, status: 'live', updatedAt: new Date().toISOString(), latencyMs: latency, detail: 'NASA EONET open-event feed connected.' });
    return (d.events ?? []).map((e) => { const g = e.geometry?.[e.geometry.length - 1]; const [lon, lat] = g?.coordinates ?? []; return { id: String(e.id), title: String(e.title ?? 'Event'), category: String(e.categories?.[0]?.title ?? 'Hazard'), lat: Number(lat), lon: Number(lon), date: g?.date ?? new Date().toISOString() }; }).filter((e) => Number.isFinite(e.lat) && e.lat >= 5 && e.lat <= 38 && e.lon >= 67 && e.lon <= 100);
}
function normaliseDistrict(v) { return v.toLowerCase().replace(/district|\s|[-_]/g, ''); }
async function fetchImdWarnings() {
    if (!IMD_VALUE) {
        setSource({ ...sourceById.imd, status: 'auth-required', detail: 'IMD exposes the required district-warning API, but this deployment has no authorised credential configured.' });
        return [];
    }
    try {
        const headers = { [IMD_HEADER]: IMD_VALUE };
        const { response, latency } = await timedFetch(`${IMD_BASE}/districtwarning`, { headers });
        const d = await response.json();
        const rows = d.data ?? d ?? [];
        setSource({ ...sourceById.imd, status: 'live', updatedAt: new Date().toISOString(), latencyMs: latency, detail: `District-wise warning API connected (${rows.length} records observed).` });
        return Array.isArray(rows) ? rows : [];
    }
    catch (e) {
        setSource({ ...sourceById.imd, status: 'error', updatedAt: new Date().toISOString(), detail: `IMD adapter error: ${e instanceof Error ? e.message : 'request failed'}` });
        return [];
    }
}
function parseCapXml(xml) {
    try {
        ...
        const d = p.parse(xml);
        const alerts = Array.isArray(d?.alert) ? d.alert : [d?.alert].filter(Boolean);
        return alerts.map((a) => { const info = Array.isArray(a.info) ? a.info[0] : a.info || {}; const area = Array.isArray(info.area) ? info.area[0] : info.area || {}; const circle = String(area.circle ?? ''); const [first] = circle.split(';'); const nums = first.replaceAll(',', ' ').split(/\s+/).map(Number); const [lat, lon, radius] = nums; const ev = String(info.event ?? 'Multi-hazard alert'); const sev = String(info.severity ?? 'Moderate').toLowerCase(); const severity = sev.includes('extreme') || sev.includes('severe') ? 'critical' : sev.includes('moderate') ? 'medium' : 'low'; return { id: String(a.identifier ?? Math.random()), title: ev, severity, region: String(area.areaDesc ?? 'India'), lat: lat || 22.59, lon: lon || 78.96, radiusKm: Number(radius || 0), timestamp: String(a.sent ?? new Date().toISOString()), source: 'NDMA SACHET', official: true, details: String(info.headline ?? info.description ?? 'Official CAP alert'), link: 'https://sachet.ndma.gov.in/' }; });
    }
    catch {
        return [];
    }
...
async function fetchNdma() {
    if (!CAP_URL && !RSS_URL) {
        setSource({ ...sourceById.ndma, status: 'configured', detail: 'SACHET adapter is ready. Configure an approved CAP XML or RSS feed URL for agency consumption.', updatedAt: null });
        return [];
    }
    const url = CAP_URL || RSS_URL;
    try {
        const started = Date.now();
        const headers = { accept: 'application/xml,text/xml,application/rss+xml' };
        if (ndmaEtag)
            headers['If-None-Match'] = ndmaEtag;
        const r = await fetch(url, { headers, signal: AbortSignal.timeout(Number(env.REQUEST_TIMEOUT_MS || 12000)) });
        const latency = Date.now() - started;
        if (r.status === 304 && ndmaXml) {
            setSource({ ...sourceById.ndma, status: 'live', updatedAt: new Date().toISOString(), latencyMs: latency, detail: 'SACHET feed unchanged (ETag 304); serving cached CAP/RSS XML.' });
            return CAP_URL ? parseCapXml(ndmaXml) : parseRssXml(ndmaXml);
        }
        if (!r.ok)
            throw new Error(`${r.status} ${r.statusText}`);
        const text = await r.text();
        ndmaXml = text;
        ndmaEtag = r.headers.get('etag') || undefined;
        ...
        setSource({ ...sourceById.ndma, status: 'error', updatedAt: new Date().toISOString(), detail: `SACHET adapter error: ${e instanceof Error ? e.message : 'request failed'}` });
        return [];
    }
}
...
function parseSdmaJson(payload) {
    ...
    }).filter((a) => a.lat >= 5 && a.lat <= 38 && a.lon >= 67 && a.lon <= 100);
}
async function fetchSdma() {
    if (!SDMA_URL) {
        setSource({ ...sourceById.sdma, status: 'configured', updatedAt: null, detail: 'State/UT SDMA adapter ready. Configure an authority-approved JSON/GeoJSON feed; no statewide endpoint is invented.' });
        return [];
    }
    try {
        const headers = { accept: 'application/json,application/geo+json' };
        if (SDMA_HEADER && SDMA_VALUE)
            headers[SDMA_HEADER] = SDMA_VALUE;
        const { response, latency } = await timedFetch(SDMA_URL, { headers });
        const payload = await response.json();
        const alerts = parseSdmaJson(payload);
        ...
        setSource({ ...sourceById.sdma, status: 'error', updatedAt: new Date().toISOString(), detail: `${SDMA_STATE} adapter error: ${e instanceof Error ? e.message : 'request failed'}` });
        return [];
    }
}
async function fetchGeoJson(url) { const { response } = await timedFetch(url); return response.json(); }
function vulnForPoint(features, lat, lon) { let best = null, bestDist = 1e9; for (const f of features) {
    const g = f.geometry;
    let p = g?.type === 'Point' ? g.coordinates : null;
    if (!p)
        continue;
    const d = Math.hypot(Number(p[1]) - lat, (Number(p[0]) - lon) * Math.cos(lat * Math.PI / 180));
    if (d < bestDist) {
        bestDist = d;
        best = f;
    }
} if (!best || bestDist > .65)
    return null; const pr = best.properties || {}; const vulnerability = Number(pr.vulnerability_score ?? pr.vulnerability ?? pr.risk ?? pr.index ?? 50); const exposure = Number(pr.population_exposure ?? pr.population ?? pr.people_at_risk ?? 0); return { vulnerability: Math.max(0, Math.min(100, Number.isFinite(vulnerability) ? vulnerability : 50)), exposure: Number.isFinite(exposure) ? exposure : 0 }; }
function asBool(v) { return v === true || v === 1 || String(v ?? '').toLowerCase() === 'true' || String(v ?? '').toLowerCase() === 'yes'; }
function parseRelief(json) {
    return (json?.features ?? []).map((f) => {
        const [lon, lat] = f.geometry?.coordinates ?? [];
        const p = f.properties || {};
        return { id: String(p.id ?? f.id ?? crypto.randomUUID()), name: String(p.name ?? p.title ?? 'Official relief site'), type: (String(p.type ?? 'shelter').toLowerCase().includes('hospital') ? 'hospital' : String(p.type ?? '').toLowerCase().includes('relief') ? 'relief_centre' : 'shelter'), lat: Number(lat), lon: Number(lon), capacity: Math.max(0, Number(p.capacity ?? 0)), occupied: Math.max(0, Number(p.occupied ?? 0)), beds: Math.max(0, Number(p.beds ?? 0)), water: asBool(p.water), power: asBool(p.power), medical: asBool(p.medical), accessibility: asBool(p.accessibility), hazardScore: Math.max(0, Math.min(100, Number(p.hazard_score ?? p.hazardScore ?? 0))), source: 'official', status: 'verified', state: String(p.state ?? ''), district: String(p.district ?? '') };
    }).filter((s) => Number.isFinite(s.lat) && Number.isFinite(s.lon) && s.lat >= 6 && s.lat <= 38 && s.lon >= 68 && s.lon <= 98);
}
function parseGsiInventory(json) {
    return (json?.features ?? []).filter((f) => { const [lon, lat] = f.geometry?.coordinates ?? []; return Number.isFinite(Number(lat)) && Number.isFinite(Number(lon)) && Number(lat) >= 5 && Number(lat) <= 38 && Number(lon) >= 67 && Number(lon) <= 100; }).map((f) => { const p = f.properties || {}; const [lon, lat] = f.geometry.coordinates; return { id: String(p.id ?? f.id ?? crypto.randomUUID()), lat: Number(lat), lon: Number(lon), title: String(p.name ?? p.title ?? p.event ?? 'GSI landslide inventory'), date: String(p.date ?? p.event_date ?? p.updatedAt ?? new Date().toISOString()) }; });
}
export async function getDashboard() {
    ...
    if (VULN_URL) {
        try {
            const j = await fetchGeoJson(VULN_URL);
            vulnFeatures = j.features ?? [];
            setSource({ ...sourceById.vulnerability, status: 'live', updatedAt: new Date().toISOString(), detail: `Configured vulnerability layer connected (${vulnFeatures.length} features).` });
        }
        catch (e) {
            setSource({ ...sourceById.vulnerability, status: 'error', updatedAt: new Date().toISOString(), detail: `Vulnerability layer: ${e instanceof Error ? e.message : 'failed'}` });
        }
    }
    else
        setSource({ ...sourceById.vulnerability, status: 'configured', detail: 'Ready for an approved population/vulnerability GeoJSON service from the deployment authority.' });
    let relief = [];
    if (RELIEF_URL) {
        try {
            relief = parseRelief(await fetchGeoJson(RELIEF_URL));
            setSource({ ...sourceById.relief, status: 'live', updatedAt: new Date().toISOString(), detail: `Official relief registry connected (${relief.length} sites).` });
        }
        catch (e) {
            setSource({ ...sourceById.relief, status: 'error', updatedAt: new Date().toISOString(), detail: `Relief registry: ${e instanceof Error ? e.message : 'failed'}` });
        }
    }
    else
        setSource({ ...sourceById.relief, status: 'configured', detail: 'Ready for an approved official shelter/relief registry GeoJSON feed.' });
    ...
    if (GSI_INVENTORY_URL) {
        try {
            const j = await fetchGeoJson(GSI_INVENTORY_URL);
            gsiInventory = parseGsiInventory(j);
            setSource({ ...sourceById.gsi, status: 'live', updatedAt: new Date().toISOString(), detail: `GSI landslide inventory connected (${gsiInventory.length} features).` });
        }
        catch (e) {
    ...
    const officialImd = imd.filter((r) => Number(r.Day1_Color || 0) >= 2).slice(0, 20).map((r) => {
        const p = INDIA_POINTS.find(x => normaliseDistrict(x.district) === normaliseDistrict(String(r.District ?? r.district ?? r.district_name ?? ''))) ?? INDIA_POINTS[0];
        const c = Number(r.Day1_Color);
    ...
    const data = { alerts, points, earthquakes, weatherByPoint: Object.fromEntries(points.map(p => [p.id, { lat: p.lat, lon: p.lon, tempC: p.tempC, precipitationMm24h: p.rainfall24h, precipitationMm72h: p.rainfall72h, windKmh: p.windKmh, humidityPct: p.humidityPct, weatherCode: 0, elevationM: p.elevationM, updatedAt: new Date().toISOString() }])), reliefSites: relief, sourceHealth: [...sourceStatus.values()], officialCoverage: { imdConfigured: Boolean(IMD_VALUE), ndmaConfigured: Boolean(CAP_URL || RSS_URL), sdmaConfigured: Boolean(SDMA_URL), bhuvanConfigured: env.BHUVAN_WMS_CONFIGURED === 'true', gsiConfigured: (env.GSI_WMS_CONFIGURED === 'true' || Boolean(GSI_INVENTORY_URL)), vulnerabilityConfigured: Boolean(VULN_URL), reliefConfigured: Boolean(RELIEF_URL) }, metrics: { activeAlerts: alerts.length, officialAlerts: alerts.filter((a) => a.official).length, peopleAtRiskEstimate: Math.round(points.reduce((sum, p) => sum + p.populationExposure * (p.riskScore / 100), 0)), provisionalRedZones: points.filter(p => p.provisionalRedZone).length, relocationCandidates: relief.length, monitoredCells: points.length }, generatedAt: new Date().toISOString() };
    cache = { expires: Date.now() + Number(env.CACHE_TTL_MS || 120000), data };
    return data;
}
export async function getWeather(lat, lon) { return { lat, lon, ...await fetchWeather(lat, lon), updatedAt: new Date().toISOString() }; }
