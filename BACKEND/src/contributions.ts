...
export function indiaCoordinate(lat:number,lon:number){return Number.isFinite(lat)&&Number.isFinite(lon)&&lat>=6&&lat<=38&&lon>=68&&lon<=98}
function distanceKm(aLat:number,aLon:number,bLat:number,bLon:number){const r=Math.PI/180;const p1=aLat*r,p2=bLat*r;const dp=(bLat-aLat)*r,dl=(bLon-aLon)*r;const h=Math.sin(dp/2)**2+Math.cos(p1)*Math.cos(p2)*Math.sin(dl/2)**2;return 6371*2*Math.atan2(Math.sqrt(h),Math.sqrt(1-h))}
function asIso(v:any){if(!v)return new Date().toISOString();if(typeof v.toDate==='function')return v.toDate().toISOString();return new Date(v).toISOString()}
function sanitise(d:any){return{id:d.id,userId:String(d.userId),userName:String(d.userName||'Verified contributor'),userEmail:d.userEmail??null,type:d.type,name:d.name,description:d.description,lat:Number(d.lat),lon:Number(d.lon),capacity:Number(d.capacity??0),state:d.state||'',district:d.district||'',status:d.status,moderationNote:d.moderationNote||'',createdAt:asIso(d.createdAt),updatedAt:d.updatedAt?asIso(d.updatedAt):undefined,duplicateScore:Number(d.duplicateScore??0)}}

export async function createContribution(req:Request){
 ...
 if(typeof name!=='string'||name.trim().length<3)throw new Error('A clear report name is required.')
 if(typeof description!=='string'||description.trim().length<12)throw new Error('Please provide enough field detail for verification.')
 const nlat=Number(lat),nlon=Number(lon),ncap=Number(capacity)
 if(!indiaCoordinate(nlat,nlon))throw new Error('Coordinates must fall inside the India monitoring envelope.')
 if(!Number.isFinite(ncap)||ncap<0||ncap>100000)throw new Error('Capacity must be between 0 and 100000.')
 const supported=new Set(['relief_centre','shelter','medical','food_water','road_blockage','flood_report','landslide_report','other']);if(!supported.has(String(type)))throw new Error('Unsupported contribution type.')
 ...
 return {id:ref.id,status:'pending' as const,duplicateFlag:duplicateScore>=50}
}

export async function myContributions(req:Request){
 const claims=await requireGoogle(req.headers.authorization)
...
export async function adminSummary(req:Request){
 ...
 const within24=(d:any)=>new Date(asIso(d)).getTime()>=cutoff
...
export async function adminContributions(req:Request,status:string){
 await requireAdmin(req.headers.authorization)
 if(!['pending','approved','rejected'].includes(status))throw new Error('Invalid moderation status.')
 const snap=await db.collection('contributions').where('status','==',status).limit(200).get()
 return snap.docs.map(d=>sanitise({id:d.id,...d.data()})).sort((a,b)=>b.createdAt.localeCompare(a.createdAt))
}

export async function moderate(req:Request,id:string){
 ...
 const ref=db.collection('contributions').doc(id);const snap=await ref.get();if(!snap.exists)throw new Error('Contribution not found.')
