...
const app=express();const port=Number(process.env.PORT||8787);const origin=process.env.CORS_ORIGIN||'http://localhost:5173'
...
function wrap(fn:(req:express.Request)=>Promise<any>){return async(req:express.Request,res:express.Response)=>{try{res.json(await fn(req))}catch(e){const msg=e instanceof Error?e.message:'Request failed';const auth=/Authentication|required|Google sign-in|admin privileges/i.test(msg);res.status(auth?401:400).json({error:msg})}}}
...
app.get('/api/weather',async(req,res)=>{const lat=Number(req.query.lat),lon=Number(req.query.lon);if(!Number.isFinite(lat)||!Number.isFinite(lon))return res.status(400).json({error:'lat and lon are required.'});try{res.json(await getWeather(lat,lon))}catch(e){res.status(503).json({error:e instanceof Error?e.message:'Weather feed unavailable.'})}})
