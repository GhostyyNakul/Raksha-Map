...
export function computeRisk(input:{rainfall24h:number;rainfall72h:number;windKmh:number;tempC:number;elevationM:number;earthquakeCount:number;history90d:number;vulnerability:number;officialWarningColor?:number}){
 ...
 const primary=primaryKey as 'flood'|'landslide'|'cyclone'|'heat'|'seismic'
 const reasons:string[]=[]
 if(input.officialWarningColor) reasons.push(`IMD warning colour ${input.officialWarningColor}`)
 if(flood>=40) reasons.push(`${Math.round(input.rainfall24h)} mm rainfall / 24h`)
 if(landslide>=40) reasons.push(`${Math.round(input.rainfall72h)} mm rainfall / 72h + elevated terrain`)
 if(cyclone>=45) reasons.push(`${Math.round(input.windKmh)} km/h wind signal`)
 if(heat>=50) reasons.push(`${input.tempC.toFixed(1)}°C heat load`)
 if(seismic>=30) reasons.push(`${input.earthquakeCount} nearby seismic event(s)`)
 if(history>=25) reasons.push(`${input.history90d} mapped hazard event(s) in recent history`)
 if(vulnerability>=45) reasons.push(`vulnerability layer score ${Math.round(vulnerability)}`)
 if(!reasons.length) reasons.push('No elevated live hazard signal')
